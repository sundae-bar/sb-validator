...
export const submitSn121Weights = async (
  ...
  try {
    ...
    if (
      ...
      (error.message.includes('not registered') || error.message.includes('validator permit'))
    ) {
      logger.error(
        {
          error: error.message,
  ...
  for (let i = 0; i < weights.length; i++) {
    if (weights[i] < 0 || weights[i] > 65535) {
      ...
      throw new Error(`Weight ${weights[i]} for UID ${uids[i]} is out of u16 range (0-65535)`);
    }
  }
